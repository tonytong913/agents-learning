...
在开始任何学习会话或 wiki 更新前，先读取 `memory/MEMORY.md` 以及其中引用的记忆文件，并把这些记忆作为当前会话的有效项目偏好。

当前关键偏好：

- 学习采用讨论优先：先阅读相关材料并在对话中讲清楚，再整理笔记。
- 未经用户明确确认，不要更新 `wiki/`、`wiki/index.md` 或 `wiki/log.md`。
- 学习某一个目标项目时，只聚焦该项目；除非用户明确要求对比，不要主动引入另一个项目做横向比较。
...
3. **等待确认**：只有当用户明确要求整理或写入笔记时，才进入归档阶段
4. **产出/更新笔记**：将讨论内容归档到 `wiki/<topic>.md`，遵循笔记页约定
5. **更新目录**：在 `wiki/index.md` 中更新对应页面的状态和说明
6. **追加日志**：在 `wiki/log.md` 末尾追加本次学习记录
